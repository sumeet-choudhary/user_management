...
from application.role.controller import find_role_by_name, add_new_role, update_role, find_role_by_id, delete_role, get_all_company_roles
        ...
        try:
            ...
            if not already_role:
                return make_response(
                    jsonify({"message": "Old role id that you are trying to update does not exist."}), 500)
            ...
            if (already_user["company_id"] != already_role["company_id"]):
                return make_response(jsonify({"message": "You can only update your company roles."}), 500)
...
class DeleteRole(Resource):
    ...
    def delete(self, id):
        try:
            ...
            if not already_role:
                return make_response(
                    jsonify({"message": "Old role id that you are trying to delete does not exist."}), 500)

            if already_role["role_name"] == "Admin":
                return make_response(jsonify({"message": "Admin role can't be deleted."}), 500)

            if (already_user["company_id"] != already_role["company_id"]):
                return make_response(jsonify({"message": "You can only delete your company roles."}), 500)
...
api.add_resource(UpdateRole, "/role/<string:id>")
api.add_resource(DeleteRole, "/role/<string:id>")
